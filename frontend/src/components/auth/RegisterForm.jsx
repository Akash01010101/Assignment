...
/* ── Luxury Glass Input ──────────────────────────────────────── */
const GlassInput = ({ icon: Icon, label, type, placeholder, value, onChange, isPassword, showPassword, togglePassword, minLength }) => {
  ...
  return (
    <div style={{
      ...
      <div style={{ marginBottom: 'var(--space-8)' }}>
        ...
        >
          <Sparkles size={12} /> Membership Open
        </motion.div>
        ...
        >
          Create your profile to book the world's most exclusive events.
        </motion.p>
      </div>
      ...
      <form onSubmit={handleSubmit} style={{ display: 'flex', flexDirection: 'column', gap: 'var(--space-6)' }}>
        ...
        <motion.div initial={{ opacity: 0, x: -10 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: 0.6 }}>
          <GlassInput 
            icon={Lock} label="Password" type={showPassword ? 'text' : 'password'} placeholder="Min 8 characters"
          ...
          {password.length > 0 && (
            <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} style={{ display: 'flex', alignItems: 'center', gap: 'var(--space-2)', marginTop: 'var(--space-3)' }}>
              <div style={{ flex: 1, display: 'flex', gap: 4 }}>
                {[1, 2, 3].map((level) => (
                  <div key={level} style={{
                    flex: 1, height: 4, borderRadius: 2,
                    background: passwordStrength >= level ? strengthColors[passwordStrength] : 'rgba(255,255,255,0.1)',
                    transition: 'background 0.3s ease',
                  }} />
                ))}
              </div>
              <span style={{ fontSize: 'var(--text-xs)', color: strengthColors[passwordStrength], fontWeight: 600, textTransform: 'uppercase', letterSpacing: '0.05em' }}>
        ...
        <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.7 }} style={{ marginTop: 'var(--space-2)' }}>
      ...
      >
        Join 50,000+ elite attendees worldwide
      </motion.div>
    </div>
  );
};
